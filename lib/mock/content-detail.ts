/**
 * コンテンツ詳細ページ（記事詳細）用の本文モック。
 *
 * 一覧用の {@link Content}（タイトル・サブタイトル・著者・サムネ・タグ等）を補完し、
 * 詳細ページにのみ必要な本文・目次・統計・コメントを保持する。認証/DB 未確定の方針に従い
 * すべて静的モックで、`id` をキーに参照する。
 */

/** コメント投稿者。アバター読み込み失敗時は initials を Avatar フォールバックに表示する。 */
type CommentAuthor = {
  name: string;
  avatar: string;
  initials: string;
...
/** 本文の 1 セクション（概要・導入のポイントなど）。`id` は目次アンカーに対応する。 */
...
/**
 * 記事詳細の本文・統計・目次・コメント。
 * 一覧の {@link Content} と `id` で対応づけ、詳細ページで両者を結合して描画する。
 */
...
/**
 * 記事 id をキーにした詳細データ。デザイン原稿に一致する `id: "1"` を中心に、
...
/**
 * 指定 id の記事詳細を返す。未定義（記事以外、または未作成の id）の場合は `undefined`。
 *
